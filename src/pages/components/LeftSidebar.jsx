...
const LeftSidebar = ({ chapters, onSelectParagraph, activeParagraphId, isPrayerPage = false }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 top-16 z-50 w-80 bg-white/80 dark:bg-dark-bg/95 backdrop-blur-xl border-r border-gold-primary/20 dark:border-dark-border/50 h-[calc(100vh-64px)] transform transition-transform duration-500 ${
          isSidebarOpen
            ? 'translate-x-0 overflow-hidden shadow-2xl lg:shadow-none'
            : '-translate-x-full lg:-translate-x-full pointer-events-none'
        } flex flex-col font-inter lg:relative lg:top-0 lg:left-0 lg:z-10 lg:h-[calc(100vh-64px)] lg:w-full`}
      >
